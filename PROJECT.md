...
- **Header button swap on collapse:** open panel shows only `x`; collapsed panel hides `x` and shows `⏻` instead (`.collapsed` class set by `toggleCategory()`; CSS at `.category:not(.collapsed):not(.cat-disabled) .power-btn`). **Exception:** a **disabled** (powered-off) category shows **only** its `⏻` in both states — the `x` is hidden while disabled (`.category.cat-disabled .header-btns button.danger`) and the `⏻` stays visible while expanded (`:not(.cat-disabled)` on the power hide rule). Clicking `⏻` re-enables it, which brings the `x` back. So: to power a category OFF it must be collapsed first, but a category that is already OFF can be powered back ON while expanded.
